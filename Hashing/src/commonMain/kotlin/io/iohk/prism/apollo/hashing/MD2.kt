...
final class MD2 : HashingBase(), HMACInterface {
    ...
    override fun processBlock(data: ByteArray) {
        ...
        for (i in 0..15) {
            ...
            x[16 + i] = u
            x[32 + i] = x[i] xor u
            tL = io.iohk.prism.apollo.hashing.MD2.Companion.S[u xor tL].let { c[i] = c[i] xor it; c[i] }
        }
        l = tL
        var t = 0
        for (j in 0..17) {
            var k = 0
            while (k < 48) {
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 0] = x[k + 0] xor it; x[k + 0] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 1] = x[k + 1] xor it; x[k + 1] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 2] = x[k + 2] xor it; x[k + 2] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 3] = x[k + 3] xor it; x[k + 3] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 4] = x[k + 4] xor it; x[k + 4] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 5] = x[k + 5] xor it; x[k + 5] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 6] = x[k + 6] xor it; x[k + 6] }
                t = io.iohk.prism.apollo.hashing.MD2.Companion.S[t].let { x[k + 7] = x[k + 7] xor it; x[k + 7] }
                k += 8
            }
            t = t + j and 0xFF
        }
